...
Python Kafka producer using Confluent Kafka libraries with support for OAuth authentication and flexible rate control.
...
### Basic Deployment (Unsecured Kafka)

```bash
kubectl apply -f producer-deployment.yaml
```

### OAuth-Secured Kafka Deployment
...
### Example 2: Fixed Rate Mode (OAuth-Secured)

Generates sensor data at a fixed rate with OAuth authentication.
...
### Example 3: Kubernetes Deployment with OAuth
...
All messages use the same randomized sensor data format.
